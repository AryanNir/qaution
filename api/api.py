...
def check_ip_reputation(ip: str, x_key: str = Header(None)):
    try:
        response = requests.get(f"https://redirect-tracker-api.onrender.com/check-ip-reputation?ip=${ip}", 
...
def check_ssl(url: str):
    try:
        ...
        response = requests.get("https://redirect-tracker-api.onrender.com/check-ssl?url=${url}",
